...
SV2V = /home/lucien/git/sv2v/bin/sv2v
YOSYS = yosys
VERILATOR = verilator
CXX = g++
...
IN_FILES := $(shell find aes_hdl_51_noia -type f -name "*.v" ! -name "tb_*.v" ! -name "final*.v")

noia41:
	IN_FILES="$(IN_FILES)" TOP_MODULE="MSKaes_128bits_round_based" OUT_BASE="syn/syn_MSKaes_128bits_round_based" LIBERTY="stdcells.lib" yosys synth.tcl -t -l "syn/syn_MSKaes_128bits_round_based_2_log.txt"

noia41_sbox:
	IN_FILES="$(IN_FILES)" TOP_MODULE="aes_sbox" OUT_BASE="syn/syn_aes_sbox" LIBERTY="stdcells.lib" yosys synth.tcl -t -l "syn/syn_aes_sbox_2_log.txt"

SHARES=2

resdir:
	bash -c 'for variant in aes_hdl_41_noia aes_hdl_41_pini aes_hdl_51_noia aes_hdl_51_pini compress; do for dir in o1 o2 o3 o4 o5 o6 o7 o8 o9 o10; do mkdir -p "syn/$${variant}/sbox/$${dir}"; done; done'
# 	for variant in 41_noia 41_pini 51_noia 51_pini; do
# 	    for dir in o1 o2 o3 o4; do
# 	        mkdir -p "syn/aes_hdl_${variant}/sbox/${dir}"
# 	        mkdir -p "syn/aes_hdl_${variant}/${dir}"
# 	    done
# 	done

# aes_hdl_%/sbox/aes_sbox: 
# 	YOSYS_DEFINES = IN_FILES=$(shell find aes_hdl_$* -type f -name "*.v" ! -name "tb_*.v" ! -name "final*.v")
# 	YOSYS_DEFINES += SHARES=$(SHARES)


# aes_hdl_%/sbox/aes_sbox: 
# # 	SHARES=$(SHARES)
# 	export $(YOSYS_DEFINES); IN_FILES="$(IN_FILES)" TOP_MODULE="aes_sbox" OUT_BASE="syn/aes_hdl_$*/sbox/" LIBERTY="stdcells.lib" yosys synth.tcl -t -l "syn/$@_$(SHARES)_log.txt"

# aes_hdl_%/MSKaes_128bits_round_based:	
resdirfullaes:
	bash -c 'for variant in aes_hdl_41_noia aes_hdl_41_pini aes_hdl_51_noia aes_hdl_51_pini compress; do for dir in o1 o2 o3 o4 o5 o6 o7 o8 o9 o10; do mkdir -p "syn/$${variant}/full_aes/$${dir}"; done; done'
...
aes_hdl_%/sbox/aes_sbox:
	...
	yosys synth.tcl -t -l "syn/$@_$(SHARES)_log.txt"

compress/sbox:
	IN_FILES="$(shell find compress -type f -name "*.v" )" \
	SHARES=$(SHARES) \
	TOP_MODULE="canright" \
	OUT_BASE="syn/compress/sbox" \
	LIBERTY="stdcells.lib" \
	yosys synth.tcl -t -l "syn/$@_$(SHARES)_log.txt"
...
aes_hdl_%/full_aes/wrapper_aes128:
	...
	OUT_BASE="syn/aes_hdl_$*/full_aes/" \
	LIBERTY="stdcells.lib" \
	yosys synth.tcl -t -l "syn/$@_$(SHARES)_log.txt"
